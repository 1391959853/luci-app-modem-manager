...
[![GitHub release](https://img.shields.io/github/v/release/1391959853/luci-app-modem-manager)](https://github.com/1391959853/luci-app-modem-manager/releases)
[![GitHub license](https://img.shields.io/github/license/1391959853/luci-app-modem-manager)](https://github.com/1391959853/luci-app-modem-manager/blob/main/LICENSE)

OpenWrt/LEDE 下的一款功能全面的 4G/5G 模块管理插件，支持移远（Quectel）和广和通（Fibocom）等多款模块，提供状态监控、AT 命令调试、短信收发等一站式管理功能。

![dashboard](docs/images/dashboard.png) *(示例截图)*
...
1. 从 [Releases](https://github.com/1391959853/luci-app-modem-manager/releases) 页面下载最新 `.ipk` 文件。
2. 上传至路由器，执行：
   ```bash
...
若希望在状态页显示运营商 Logo，请将以下图片文件放入路由器的 /www/luci-static/resources/operator/ 目录：
...
图片建议尺寸 32×32 或 64×64，背景透明。若无图片，插件将只显示文字，不影响使用。
...
插件默认使用 /etc/config/modem 存储配置，目前主要用于缓存 AT 命令输出。未来可扩展 APN 等选项。

开机自动加载驱动

启动脚本 /etc/init.d/modem_driver 会在开机时检测 USB 设备并加载所需驱动。如需添加更多模块的 USB ID，可编辑该脚本的 case 分支。
...
本项目基于 GPL-2.0-or-later 许可证开源，详情见 LICENSE 文件。
